...
REGEX_FOR_JOB_EXPERIENCE = r"[А-я]+\s[0-9]{4}\s\—\s[А-я]+\s[0-9]{4}" \
                           r"|[А-я]+\s[0-9]{4}\s\—\sнастоящее\sвремя" \
                           r"|[А-я]+\s[0-9]{4}\s"

REGEX_FOR_FIRM_NAME = r"[0-9]+\sгода\s[0-9]+\sмесяца([А-я\s]+\n?)" \
...
REGEX_FOR_JOB_PERIOD = r"[0-9]+\sгода\s[0-9]+\sмесяца" \
